...
//    @Inject
    ...
    public void setSaveCommitteeDecision(
            ...
            String Notes){
        disposable.add(apiInterface.saveCommitteeDecision_Manufacturing(LocaleHelper.getLanguage(getApplication().getApplicationContext()),userId,deviceSerialNo,rejectionRequestId,OkQty,OkBasketCode,RejectedQty,DepartmentId,RejectionReasonID,Notes)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe( __ -> status.postValue(Status.LOADING))
                .subscribe(
