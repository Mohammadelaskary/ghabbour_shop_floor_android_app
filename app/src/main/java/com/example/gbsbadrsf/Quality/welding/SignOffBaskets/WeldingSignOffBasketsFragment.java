...
public class WeldingSignOffBasketsFragment extends Fragment implements BarcodeReadListener, OnBasketRemoved, View.OnClickListener, BarcodeReader.BarcodeListener, BarcodeReader.TriggerListener {
...
//                            binding.addOkBasketBottomSheet.totalAddedQtn.setText(basketQty);
    ...
    private WeldingOkBasketListAdapter adapter;
    private void setUpOkBasketsRecyclerView() {
        adapter = new WeldingOkBasketListAdapter(okBasketList,this,isBulk);
    ...
    private void initializeBottomSheets() {
        ...
        addOkBasketsBottomSheet.addBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            @Override
            public void onStateChanged(@NonNull View bottomSheet, int newState) {
                if (newState==BottomSheetBehavior.STATE_HIDDEN)
                    binding.disableColor.setVisibility(View.GONE);
    ...
    private  void getData() {
        if (getArguments()!=null){
            ...
            if (remainingQty==0){
                ...
                binding.signOffBaskets.addOkBasket.setTextColor(getActivity().getResources().getColor(R.color.disable));
            } else {
                binding.signOffBaskets.addOkBasket.setEnabled(true);
                binding.signOffBaskets.addOkBasket.setTextColor(getActivity().getResources().getColor(R.color.colorPrimary));
            }
        }
    }
        ...
        switch (v.getId()){
            case R.id.save:
                if (defectedQty!=0&&defectedBasket.isEmpty())
                    ...
                    FullInspectionData_Welding data = new FullInspectionData_Welding(
                            USER_ID,
                            DEVICE_SERIAL_NO,
