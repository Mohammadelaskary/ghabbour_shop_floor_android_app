...
import com.example.gbsbadrsf.AddWorkers.MachineData;
...
import com.example.gbsbadrsf.databinding.FragmentAddStationWorkersBinding;
...
public class AddWorkersFragment extends Fragment implements View.OnClickListener, AllWorkersAdapter.WorkerItemClicked, BarcodeReadListener, BarcodeReader.BarcodeListener, BarcodeReader.TriggerListener {
    ...
    private FragmentAddStationWorkersBinding binding;
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        binding = FragmentAddStationWorkersBinding.inflate(inflater,container,false);
    ...
    private void observeGettingMachineData() {
        viewModel.getGetStationWorkers().observe(getViewLifecycleOwner(), apiResponseGetMachineWorkers -> {
            if (apiResponseGetMachineWorkers!=null){
                ...
                if (apiResponseGetMachineWorkers.getResponseStatus().getIsSuccess()){
                    stationData = apiResponseGetMachineWorkers.getStationParam();
    ...
    private void fillData(ApiResponseGetStationWorkers response) {
        binding.parentDesc.setText(response.getParentDescription());
        binding.jobordernum.setText(response.getStationParam().getJobOrderName());
    ...
    public void onBarcodeEvent(BarcodeReadEvent barcodeReadEvent) {
        getActivity().runOnUiThread(()->{
            ...
            if (binding.dataLayout.getVisibility()==View.GONE) {
                ...
                viewModel.getStationWorkers(scannedText);
            } else {
    ...
    public void onClick(View v) {
        if (!selectedWorkersIds.isEmpty()){
            AddWorkers_StationData data = new AddWorkers_StationData(
                    USER_ID,
                    DEVICE_SERIAL_NO,
                    binding.stationCode.getEditText().getText().toString().trim(),
                    selectedWorkersIds,
                    LocaleHelper.getLanguage(getContext())
            );
            viewModel.addStationWorkers(data);
        } else {
